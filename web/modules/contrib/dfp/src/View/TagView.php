...
class TagView {
   ...
   */
  public static function formatSize($sizes) {
    $processed_sizes = array_map(
      function ($raw_size) {
        $raw_size_trimmed = trim($raw_size);

        if ($raw_size_trimmed === 'fluid') {
          // If the ad sizes string contains the special keyword "fluid," use
          // the Google NamedSize ['fluid']. See DFP documentation at
          // https://developers.google.com/doubleclick-gpt/reference?rd=1#type-definitions
          // Note that both 'fluid' and ['fluid'] are acceptable forms to
          // declare a slot size as fluid.
          return 'fluid';
        }
        elseif (preg_match('/^\d+x\d+$/', $raw_size_trimmed)) {
          return array_map('intval', explode('x', $raw_size_trimmed));
        }

        // Both <none> and invalid sizes are handled here.
        return FALSE;
      },
      explode(',', $sizes)
    );

    // Filter out leftovers from non-valid sizes specifications.
    $processed_sizes = array_values(array_filter($processed_sizes, function ($size) {
      return $size !== FALSE;
    }));
    if (count($processed_sizes) === 1 && is_array($processed_sizes[0])) {
      $processed_sizes = reset($processed_sizes);
    }
    $output_sizes = json_encode(
      $processed_sizes,
      JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT
    );
    $output_sizes = str_replace(',', ', ', $output_sizes);

    return Markup::create(Xss::filter($output_sizes));
  }
  ...
  public function getClickUrl() {
    ...
    if (is_null(self::$clickUrl)) {
      ...
      if (self::$clickUrl && !preg_match("/^https?:\/\//", self::$clickUrl)) {
        self::$clickUrl = Url::fromUserInput(self::$clickUrl, ['absolute' => TRUE])
          ->toString();
      }
    }
  ...
  /**
   * Gets whether rendering is set to async mode.
   *
   * @return bool
   *   TRUE if the rendering mode is set to async, FALSE otherwise.
   */
  public function isAsyncMode() {
    return $this->globalSettings->get('async_rendering');
  }
