...
class AdminSettings extends ConfigFormBase {
   ...
   */
  public function __construct(ConfigFactoryInterface $config_factory, TypedConfigManager $config_typed, EntityTypeBundleInfoInterface $bundle_info) {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['global_tag_settings']['network_id'] = [
      ...
      '#required' => TRUE,
      '#description' => $this->t('The Network ID to use on all tags. According to Google this value should begin with a /.'),
    ];
    ...
    $form['global_tag_settings']['async_rendering'] = [
      '#type' => 'checkbox',
      '#title' => $this->t('Render ads asynchronously'),
      '#default_value' => $config->get('async_rendering'),
      '#description' => $this->t('This can speed up page rendering time by loading page content without waiting for ads to load. For proper compatibility with current DFP/Ad Manager, make sure to enable it.'),
    ];
